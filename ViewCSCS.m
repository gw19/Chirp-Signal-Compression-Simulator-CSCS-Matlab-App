classdef ViewCSCS < handle
    ...
    methods (Access = protected)
        ...
        function updatePlot(app)
            ...
            xlim(app.UISt, [t(1)*scale, t(end)*scale]);
            ylim(app.UISt, [-max(abs(St))-0.5, max(abs(St))+0.5]);
            
            % s_output(t)
            plot(app.UISot, t*scale, real(Sot)./Fsn);
            ...
            ttd = tt2 - tt1;
            zoomin = 0.2;
            plot(app.UISotdb, t*scale, Sotdb);
            ylabel(app.UISotdb, 'Sout(t)');
            axis(app.UISotdb, 'tight');
            xlim(app.UISot, [(tt1 - ttd/zoomin)*scale, (tt2 + ttd/zoomin)*scale]);
            xlim(app.UISotdb, [(tt1 - ttd/zoomin)*scale, (tt2 + ttd/zoomin)*scale]);
            %xlim(app.UISotdb, [(t(1)+zoomin*T)*scale, (t(end)-zoomin*T)*scale]);
            ...
            if sum(swTarget_Value) >= 2
                ...
                for ii = 1 : sum(swTarget_Value)
                    plot(app.UIStm, tm*scale, real(Srt(ii,:)-2*(ii-1)));
                    hold(app.UIStm, 'on');
                end
                axis(app.UIStm, 'tight');
                %xlim(app.UIStm, [tm(1)*scale, tm(end)*scale]);
                
                plot(app.UIStc, tm*scale, real(Stc));
                axis(app.UIStc, 'tight');
                %xlim(app.UIStc, [tm(1)*scale, tm(end)*scale]);
                ...
                plot(app.UISot, tm*scale, real(Sotc)./Fsn);
                axis(app.UISot, 'tight');
                xlim(app.UISotdb, [(ttm1 - ttmd/zoomin)*scale, (ttm2 + ttmd/zoomin)*scale]);
                xlabel(app.UISot, 'Time [\mus]');
                
                % s_output(t) in [dB]
                plot(app.UISotdb, tm*scale, Sotcdb);
                ylabel(app.UISotdb, 'S_o_u_t(t)');
                axis(app.UISotdb, 'tight');
                xlim(app.UISot, [(ttm1 - ttmd/zoomin)*scale, (ttm2 + ttmd/zoomin)*scale]);
                xlim(app.UISotdb, [(ttm1 - ttmd/zoomin)*scale, (ttm2 + ttmd/zoomin)*scale]);
                ylim(app.UISotdb, [-30, 0]);
                ...
                hold(app.UIStm, 'off');
                % each s(t)
                plot(app.UIStm, t*scale, real(St));
                xlim(app.UIStm, [t(1)*scale, t(end)*scale]);
                ylim(app.UIStm, [-max(abs(St))-0.5, max(abs(St))+0.5]);
                
                % combined s(t)
                plot(app.UIStc, t*scale, real(St));
                xlim(app.UIStc, [t(1)*scale, t(end)*scale]);
    ...
    methods (Access = protected)
        ...
        function createComponents(app)
            ...
            app.Label34.Text = 'Set Height and Angle of Incidence';
            ...
            app.TextArea2.Value = {'Second, assume that all of the targets are '; 'on the ground range, input the array height, '; 'incidence angle, and ranges between the '; 'near range and targets, check if it can be '; 'identified by compressing the combination '; 'signal after the matched filter.'};
            ...
            title(app.UIStm, 'Multiple-Targets Echo Signals');
